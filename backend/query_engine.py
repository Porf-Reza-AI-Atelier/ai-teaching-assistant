...
class EnhancedQueryEngine:
    def __init__(self, course_id: str = "demo"):
        ...
        try:
            ...
            self.vector_store = QdrantVectorStore(
                ...
                enable_hybrid=False  # Disable hybrid search to avoid issues
            )
            ...
            try:
                ...
                print(f"⚠️ Re-ranker initialization failed: {rerank_error}")
                print("   Continuing without re-ranking...")
                self.reranker = None
            
            # Create query engine with or without re-ranking
            postprocessors = [self.reranker] if self.reranker else []
            self.query_engine = self.index.as_query_engine(
                similarity_top_k=10 if self.reranker else 5,
    ...
    def query(self, question: str, lesson_filter: Optional[int] = None, 
              document_filter: Optional[str] = None, category_filter: Optional[str] = None) -> Dict:
        """
        Enhanced query with optional filtering by lesson, document, or category
        ...
        Args:
            ...
            category_filter: Optional category filter ("lesson", "general", or None for all)
        """
        ...
        try:
            ...
            filters = self._build_metadata_filters(lesson_filter, document_filter, category_filter)
            ...
            if hasattr(response, 'source_nodes') and response.source_nodes:
                ...
                for i, node in enumerate(response.source_nodes):
                    try:
                        ...
                        print(f"⚠️ Error extracting source info for node {i}: {e}")
                        continue
            else:
            ...
            return {
                "question": question,
                "answer": f"I apologize, but I encountered an error processing your question: {str(e)}. Please try rephrasing your question or contact support.",
                "response_time": 0,
                "sources": [],
                "error": str(e),
                "timestamp": time.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def _build_metadata_filters(self, lesson_filter: Optional[int], 
                               ...
                               category_filter: Optional[str]) -> Optional[MetadataFilters]:
    ...
    def _extract_source_info(self, node) -> Dict:
        """Extract enhanced source information from node with error handling"""
        try:
            # Handle both direct storage and LlamaIndex storage formats
            ...
            if hasattr(node, 'text') and node.text:
                ...
                node_text = metadata['text']
            elif metadata.get('_node_content'):
                node_text = metadata['_node_content']
            
            # Create display name with hierarchy
            if metadata.get('lesson_order', 0) == 0:
                # General document
                display_name = f"📄 General: {metadata.get('document_name', 'Unknown Document')}"
                if metadata.get('subfolder'):
                    display_name = f"📁 {metadata.get('subfolder')}: {metadata.get('document_name', 'Unknown Document')}"
            else:
                # Lesson document
            ...
            elif hasattr(node, 'node') and hasattr(node.node, 'score') and node.node.score is not None:
                confidence = float(node.node.score)
            ...
            return {
                ...
                "confidence": 0.0,
                "content_preview": "Could not extract content preview",
    ...
    def _get_course_context(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "general_documents": list(general_docs),
    ...
    def query_general_materials(self, question: str) -> Dict:
        """Convenience method to query only general course materials (syllabus, etc.)"""
        return self.query(question, category_filter="general")

    def query_lesson_materials(self, question: str) -> Dict:
        """Convenience method to query only lesson-specific materials"""
        return self.query(question, category_filter="lesson")
    ...
    def get_course_summary(self) -> Dict:
        ...
        try:
            ...
            General Materials: {len(context.get('general_documents', []))} documents
